        ...
        public class MancalaDataStructure implements Serializable{
            ...
            private static final int PLAYER_TWO = 13;
             private  int pitBeginning = 4;  //not final because we might want a different size board in the future
            ...
            public Store getStore(final int pitNum){
                return (Store) data.get(storePos(pitNum));
            }
            ...
            private int pitPos(final int pitNum) {
             ...
             /**
     * Returns the position of the store for a given player.
     *
     * @param playerNum The player number (1 or 2).
     * @return The index of the store in the data list.
     */
    public int getStorePlayerPos(final int playerNum) {
        if (playerNum == 1) {
            return PLAYER_ONE;
        } else if (playerNum == SECOND_PLAYER) {
            return PLAYER_TWO;
        } else {
            throw new IllegalArgumentException("Invalid player number: " + playerNum);
        }
    }
    public boolean isStore(final int pos) {
        return pos == PLAYER_ONE || pos == PLAYER_TWO;
    }
            ...
            public void setUpPits() {
                for (int i = 0; i < PLAYER_ONE; i++) {
                    data.get(i).addStones(pitBeginning);
                }
